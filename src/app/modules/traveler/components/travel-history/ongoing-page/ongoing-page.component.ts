...
@Component({
  ...
  styleUrl: './ongoing-page.component.scss'
})
export class OngoingPageComponent implements OnChanges, AfterViewInit {
  ...
  getTravels(user_id: string) {
    ...
    // --> 2 En curso
    // --> 3 Completado
    this.tripTravelerService.getTravels(user_id, 'ongoing' ).subscribe(
      {
        next: (data) => {
