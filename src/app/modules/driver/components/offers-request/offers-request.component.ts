...
import { DataDriverService } from '../../../../services/data/data_driver.service';
// -- Services

@Component({
  selector: 'app-offers-request',
  standalone: true,
  imports: [FooterPageComponent, MapPageComponent],
  templateUrl: './offers-request.component.html',
  styleUrl: './offers-request.component.scss'
})
export class OffersRequestComponent implements OnInit, AfterViewInit, OnChanges {


  title_page = "Aceptar Oferta de Viaje";
  lat: number = 23.0415;
  lon: number = -81.5775;
  ...
  async ngOnInit() {
    ...
    this.user_type = this.userData.user_type === 'traveler' ? 'Viajero' : 'Conductor';
  ...
  subscribeLocation() {
    ...
    this.dataDriverService.getLocation().subscribe(data => {
  ...
  async getLocation() {
    ...
    this.geolocService.startWatchingPosition((position: Coordinate) => {
      ...
      if (Math.floor(this.lat * 10000) !== Math.floor(position[1] * 10000) || Math.floor(this.lon * 1000) !== Math.floor(position[0] * 1000)) {
        ...
        this.dataDriverService.setLocation(position);
  ...
  connectSocket() {
    ...
    this.socket = this.socketioService.connectSocket(environment.socketUrl+'/offer');

    this.socketioService.get_socketStatus(this.socket).subscribe(
      status => {
        this.dataDriverService.setSocketOffer(status);
      }
    );
  ...
  subscribeSocketOffer() {

    this.dataDriverService.getSocketOffer().subscribe(data => {

      console.log('subscribeSocketOffer', data)
      if (data !== undefined) {
        this.socket_offer$ = data;
      }
  ...
  disconnectSocket() {
    if (this.socket) {
      ...
      this.dataDriverService.setSocketOffer(false);
